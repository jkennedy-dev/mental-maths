...
# ---------------------------------------------------------------------------
# Import the package without executing curses.wrapper at module level.
# The package is named "mental-maths" (hyphen), so we load it manually.
# ---------------------------------------------------------------------------

_PKG_PATH = Path(__file__).parent.parent / "mental-maths" / "__main__.py"

def _load_module():
    spec = importlib.util.spec_from_file_location("mental_maths_main", _PKG_PATH)
...
OpConfig      = m.OpConfig
Question      = m.Question
generate_question = m.generate_question
check_answer  = m.check_answer
_rnd          = m._rnd
_fmt          = m._fmt
_cfg_to_dict  = m._cfg_to_dict
_dict_to_cfg  = m._dict_to_cfg
_load_data    = m._load_data
_save_data    = m._save_data
_make_session = m._make_session
_build_rows   = m._build_rows
_q_line       = m._q_line
...
class TestGenerateQuestion:
    ...
    def test_subtraction_allows_negative(self):
        ...
        with patch.object(random, 'randint', side_effect=[3, 7]):  # a=3, b=7 → answer=-4
            q = generate_question(cfg)
        assert q.answer < 0
